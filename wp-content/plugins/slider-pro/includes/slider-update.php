...
/**
* When the update cycle runs, if there is any sldier update available append its information
*/
function sliderpro_update_check($transient) {
	...
	if ($response !== false && version_compare($current_version, $response->new_version, '<')) {
		if (SLIDERPRO_AUTOMATIC_UPDATE === true)
			$response->package = get_option('slider_pro_download_url');
...
function sliderpro_update_info($false, $action, $args) {
	...
	if ($response !== false) {
		if (SLIDERPRO_AUTOMATIC_UPDATE === true)
			$response->download_link = get_option('slider_pro_download_url');
		...
		if (!is_wp_error($request) && wp_remote_retrieve_response_code($request) == 200) {
			...
			if (!empty($data)) {
				...
				if (!empty($data->{'download-purchase'}->{'download_url'})) {
					...
					update_option('slider_pro_download_url', $data->{'download-purchase'}->{'download_url'});
					
					// update the automatic update status to 'enabled'
