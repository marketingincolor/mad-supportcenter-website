	...
	function AdvancedSlider(instance, options) {
		...
		*/
		...
		function parseHTML() {
			...
			if (isOldIE) {
				slides.find('img.thumbnail').parent('a').css('display', 'none');
				slides.find('img.thumbnail').css('display', 'none');
		...
		*/
		function doSliderLayout() {
			...
			if ((self.settings.thumbnailType == 'scroller' || self.settings.thumbnailType == 'tooltipAndScroller') && !self.settings.thumbnailScrollerOverlay) {
		...
		*/
		function createThumbnailScroller() {
			...
			if (self.settings.thumbnailTooltip)
				$('<div class="tooltip"><div class="content"></div></div>').hide().appendTo($('body'));
		...
		*/
		function showTooltip(index) {
			var thumbnail = thumbnailScroller.find('.thumbnail').eq(index),
			...
			var tooltipLeft = tooltip.outerWidth() / 2,
				tooltipTop = tooltip.outerHeight() + parseInt(tooltip.css('marginBottom'));
		...
		*/
		function hideTooltip(index) {
			...
			var tooltip = $('body').find('.tooltip'),
				...
				thumbnail = thumbnailScroller.find('.thumbnail').eq(index);
